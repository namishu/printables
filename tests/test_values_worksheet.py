...
from namishu_printables.values_worksheet import ValuesWorksheetApp
from namishu_printables.values_worksheet import app as app_module
...
def test_file_order_blank_lines_duplicates_and_naming(tmp_path, monkeypatch):
    ...
    assert output == tmp_path / "values-worksheet-家庭.pdf"
...
def test_invalid_inputs_preserve_output(tmp_path, content, error):
    ...
    with pytest.raises(ValueError, match=error):
        ValuesWorksheetApp().generate(file_path=source, output_path=output)
...
def test_invalid_config_or_layout(tmp_path, override):
    with pytest.raises(ValueError):
        ValuesWorksheetApp(config(tmp_path, override)).plan()
...
def test_too_long_to_fit_and_item_limits_override(tmp_path):
    ...
    with pytest.raises(ValueError, match="one page"):
        ValuesWorksheetApp().plan(file_path=source)
    source.write_text("健康\n" * 101, encoding="utf-8")
    plan = ValuesWorksheetApp(config(tmp_path, {"limits": {"max_items": 101}, "grid": {"row_gap_mm": 2}})).plan(
        file_path=source
    )
...
def test_small_list_and_configurable_columns(tmp_path):
    ...
    assert ValuesWorksheetApp().plan(file_path=source).columns == 3
    plan = ValuesWorksheetApp(config(tmp_path, {"grid": {"columns": 2}})).plan(file_path=source)
...
def test_font_shrinks_within_readability_limit(tmp_path):
    source = tmp_path / "input.txt"
    source.write_text("健康\n" * 100, encoding="utf-8")
    plan = ValuesWorksheetApp(config(tmp_path, {"grid": {"row_gap_mm": 5}})).plan(file_path=source)
    assert 12 <= plan.font_size_pt < 18
...
def test_drawing_failure_preserves_destination(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="draw failed"):
        ValuesWorksheetApp().generate(output_path=output)
    assert output.read_bytes() == b"existing"
...
def test_adaptive_column_gaps_centering_and_separators(tmp_path):
    ...
    app = ValuesWorksheetApp(config(tmp_path, overrides))
    plan = app.plan()
    ascent, descent = pdfmetrics.getAscentDescent(plan.font, plan.font_size_pt)
...
def test_single_column_centered(tmp_path):
    ...
    plan = ValuesWorksheetApp(config(tmp_path, {"grid": {"columns": 1}})).plan(file_path=source)
