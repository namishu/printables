...
def draw_page(pdf: Canvas, plan: PagePlan) -> None:
    ...
    for boxes, style in ((plan.cards, card), (plan.labels, title)):
        ...
        for box in boxes:
            pdf.roundRect(box.x_mm * mm + stroke / 2, box.y_mm * mm + stroke / 2,
                          box.width_mm * mm - stroke, box.height_mm * mm - stroke,
                          style["border_radius_mm"] * mm, stroke=1, fill=0)
    ...
    pdf.rect(text.x_mm * mm - 2 * mm, text.y_mm * mm + desc - mm,
             width + 4 * mm, asc - desc + 2 * mm, fill=1, stroke=0)
    pdf.setFillColor(HexColor(cut["watermark_color"]))
    pdf.setFont(plan.font, text.size_pt)
    pdf.drawString(text.x_mm * mm, text.y_mm * mm, text.value)
    icon("cut", cut["icon_inset_mm"], middle - cut["icon_size_mm"] / 2,
         cut["icon_size_mm"], cut["icon_size_mm"])
