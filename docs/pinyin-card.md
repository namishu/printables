<h1 align="center">拼音卡</h1>

<p align="center">
  <a href="../examples/pinyin-card/pinyin-card.pdf">
    <img src="../examples/pinyin-card/preview.png" alt="韵母页预览，统一字号、四行六列排列" width="420">
  </a>
</p>

拼音卡将声母、韵母和整体认读音节分别排成一页 A4 横向 PDF，适合放在书桌旁做认读参考，
也适合亲子指读、课堂领读和拼读练习。
...
<p align="center">
  示例 PDF 文件：<a href="../examples/pinyin-card/pinyin-card.pdf">整套拼音卡</a>
</p>
...
得到一个三页的 `pinyin-card-all.pdf`，保存在运行命令的当前目录。
...
```bash
namishu-printables pinyin-card shengmu
namishu-printables pinyin-card yunmu
namishu-printables pinyin-card yinjie
```

| 类型 | 内容 | 默认输出文件 | 页数 |
| --- | --- | --- | --- |
| `shengmu` | 声母页，23 项，包含 y、w | `pinyin-card-shengmu.pdf` | 1 |
| `yunmu` | 韵母，24 项 | `pinyin-card-yunmu.pdf` | 1 |
| `yinjie` | 整体认读音节，16 项 | `pinyin-card-yinjie.pdf` | 1 |
| 不指定类别 | 以上三类，按表中顺序排列 | `pinyin-card-all.pdf` | 3 |
...
```bash
namishu-printables pinyin-card -o cards/pinyin.pdf
```
...
```bash
namishu-printables pinyin-card --config design.yaml
```

未填写的设置保留默认值，也可以下载[完整示例配置](../examples/pinyin-card/design.yaml)修改。
...
[查看声母页预览](../examples/pinyin-card/shengmu.png) · [下载声母页 PDF](../examples/pinyin-card/shengmu.pdf)。
每项拼音一个方框，`zh`、`ang`、`yuan` 等均各占一个框。三页内边距均为 3 mm，框间水平间距均为 6 mm。
[韵母页预览](../examples/pinyin-card/yunmu.png) · [整体认读音节页预览](../examples/pinyin-card/yinjie.png)。
...
内置配置见 [defualt.yaml](../src/namishu_printables/pinyin_card/config/defualt.yaml)。
